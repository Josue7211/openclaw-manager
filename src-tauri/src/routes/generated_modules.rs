...
const LOCAL_GENERATED_MODULES_OWNER_ID: &str = "__desktop_local__";
...
async fn fetch_module_row(
    ...
    let row: ModuleRow = sqlx::query_as(
        "SELECT id, user_id, name, description, icon, source, config_schema, \
         ...
         FROM generated_modules WHERE id = ? AND user_id = ?",
    )
    .bind(id)
    .bind(user_id)
    .fetch_one(db)
...
pub fn router() -> Router<AppState> {
    Router::new()
        .route("/generated-modules", get(list_modules).post(create_module))
        .route(
            "/generated-modules/{id}",
            put(update_module).delete(delete_module),
        )
        .route("/generated-modules/{id}/toggle", patch(toggle_module))
        .route("/generated-modules/{id}/versions", get(list_versions))
        .route("/generated-modules/{id}/rollback", post(rollback_module))
        .route("/generated-modules/bridge", post(bridge_proxy))
        .route(
            "/generated-modules/modules",
            get(list_modules).post(create_module),
        )
        .route(
            "/generated-modules/modules/{id}",
            put(update_module).delete(delete_module),
        )
        .route(
            "/generated-modules/modules/{id}/toggle",
            patch(toggle_module),
        )
        .route(
            "/generated-modules/modules/{id}/versions",
            get(list_versions),
        )
        .route(
            "/generated-modules/modules/{id}/rollback",
            post(rollback_module),
        )
}
...
async fn list_modules(State(state): State<AppState>) -> Result<Json<Value>, AppError> {
    ...
    .bind(LOCAL_GENERATED_MODULES_OWNER_ID)
    .fetch_all(&state.db)
    .await?;
...
async fn create_module(
    ...
    // Insert version 1
    sqlx::query(
        "INSERT INTO generated_module_versions (id, module_id, version, source, config_schema, created_at) \
         VALUES (?, ?, 1, ?, ?, ?)",
    )
    .bind(&version_id)
    .bind(&id)
    ...
    Ok(Json(json!({
        "module": {
            "id": id,
            "userId": LOCAL_GENERATED_MODULES_OWNER_ID,
            "name": body.name.trim(),
            "description": desc,
            "icon": icon,
            "source": body.source,
...
async fn update_module(
    ...
    let row: Option<(i64,)> = sqlx::query_as(
        "SELECT version FROM generated_modules WHERE id = ? AND user_id = ? AND deleted_at IS NULL",
    )
    .bind(&id)
    .bind(LOCAL_GENERATED_MODULES_OWNER_ID)
    ...
    let mut query = String::from(
        "UPDATE generated_modules SET source = ?, version = ?, config_schema = ?, updated_at = ?",
    );
    ...
    sqlx::query(
        "DELETE FROM generated_module_versions \
         WHERE module_id = ? AND version NOT IN \
         (SELECT version FROM generated_module_versions WHERE module_id = ? ORDER BY version DESC LIMIT 5)",
    )
    .bind(&id)
    .bind(&id)
    .execute(&state.db)
    ...
    let row = fetch_module_row(&state.db, &id, LOCAL_GENERATED_MODULES_OWNER_ID).await?;
...
async fn delete_module(
    ...
    let result = sqlx::query(
        "UPDATE generated_modules SET deleted_at = ?, enabled = 0, updated_at = ? \
...
async fn toggle_module(
    ...
    sqlx::query(
        "UPDATE generated_modules SET enabled = ?, updated_at = ? \
         WHERE id = ? AND user_id = ? AND deleted_at IS NULL",
    )
    .bind(enabled_val)
    .bind(&now)
...
async fn list_versions(
    ...
    let exists: Option<(String,)> =
        sqlx::query_as("SELECT id FROM generated_modules WHERE id = ? AND user_id = ?")
            .bind(&id)
            .bind(LOCAL_GENERATED_MODULES_OWNER_ID)
            .fetch_optional(&state.db)
            .await?;
    ...
    let rows: Vec<(String, String, i64, String, String, String)> = sqlx::query_as(
        "SELECT id, module_id, version, source, config_schema, created_at \
         FROM generated_module_versions \
         WHERE module_id = ? \
...
async fn rollback_module(
    ...
    let current_version = current
        .ok_or(AppError::NotFound("Module not found".into()))?
        .0;

    // Get the target version's source and config_schema
    let target: Option<(String, String)> = sqlx::query_as(
        "SELECT source, config_schema FROM generated_module_versions \
    ...
    let (source, config_schema) = target
        .ok_or_else(|| AppError::BadRequest(format!("Version {} not found", body.version)))?;
    ...
    sqlx::query(
        "UPDATE generated_modules SET source = ?, config_schema = ?, version = ?, updated_at = ? \
...
struct BridgeBody {
    ...
    command: String,
    #[allow(dead_code)]
    // deserialized from JSON request but not yet read; needed when bridge proxy is implemented
    args: Option<Value>,
}
