...
- [x] **BJORN-01**: User can describe a module in natural language via chat with Agent
- [x] **BJORN-02**: Agent generates a React component using module primitives
...
- **AISUG-02**: Agent suggests new modules based on usage patterns
...
| Plugin/extension marketplace | Requires sandboxing, API stability, review process, versioning. Agent module builder IS the extensibility system. |
| Full email client (IMAP/SMTP) | Multi-year project (Thunderbird has 20 years). Email digest module covers the use case. |
| Social media integration | API instability, content moderation liability. RSS-based widgets via Agent if needed. |
