...
This matrix is the source of truth for clawctrl Proxmox parity. A Proxmox control is not considered shipped unless it has a backend path, UI state, permission/error handling, task/audit behavior when applicable, and tests.

Status legend:

- `implemented`: works through the clawctrl backend.
- `external`: works by launching the real Proxmox UI; embedded clawctrl parity is still missing.
...
- Disposable live-write certification is scripted by `npm run proxmox:certify-live -- --yes --lifecycle`. The runner refuses to mutate a target unless VMID and exact guest name are supplied, and the name starts with `clawctrl-cert`, `cc-cert`, or `test-clawctrl` unless `PROXMOX_CERTIFY_ALLOW_ANY=I_UNDERSTAND_THIS_MUTATES_PROXMOX` is set.
...
PROXMOX_CERTIFY_NAME=clawctrl-cert-vm \
...
PROXMOX_CERTIFY_NAME=clawctrl-cert-ct \
