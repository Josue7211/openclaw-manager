#!/usr/bin/env node
/**
 * Lightweight API server for exposing harness workspace files.
 * Run this on the machine where the selected harness workspace lives.
 *
 * Environment variables:
 *   PORT                 – listen port (default: 3939)
 *   API_KEY              – bearer token for auth (optional, open if unset)
 *   HARNESS_WORKSPACE    – workspace directory (default: ~/.harness/workspace)
 *   OPENCLAW_WORKSPACE   – compatibility alias
 *
 * Then point Mission Control at it via the Settings page or keyring:
 *   HARNESS_API_URL = http://<host>:3939
 *   HARNESS_API_KEY = <your API_KEY>
 */
...
const WORKSPACE = process.env.HARNESS_WORKSPACE
  ...
  || join(homedir(), '.harness/workspace')
