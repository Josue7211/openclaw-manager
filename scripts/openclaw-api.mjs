...
const server = createServer(async (req, res) => {
  if (req.method === 'OPTIONS') {
    res.writeHead(204, { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Authorization, Content-Type', 'Access-Control-Allow-Methods': 'GET, POST, DELETE, OPTIONS' })
    return res.end()
  }
  ...
  if (path === '/file' && req.method === 'DELETE') {
    ...
    const full = safePath(filePath)
    if (!full) return json(res, { error: 'Invalid path' }, 400)
    if (!existsSync(full)) return json(res, { error: 'File not found' }, 404)
    try {
      unlinkSync(full)
      return json(res, { ok: true })
    } catch {
