...
function _setDotIndicatorsEnabled(enabled: boolean): void {
...
function _reorderPages(pageIds: string[]): void {
  _pushUndo()
...
function _resetPageLayout(pageId: string): void {
  _pushUndo()
  _cached = {
    ..._cached,
    pages: _cached.pages.map(p =>
...
function _resetAllLayouts(): void {
  _pushUndo()
  _cached = {
    ..._cached,
