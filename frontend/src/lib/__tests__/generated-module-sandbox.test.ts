...
import { buildSandboxHTML, getThemeVarsCSS } from '../generated-module-sandbox'

describe('buildSandboxHTML', () => {
  const themeCSS = ':root { --accent: #ff6b6b; }'
  const source = 'function GeneratedWidget() { return h("div", null, "Hello") }'
  ...
  it('contains the component source', () => {
    const html = buildSandboxHTML(source, themeCSS)
    expect(html).toContain('GeneratedWidget')
  })
