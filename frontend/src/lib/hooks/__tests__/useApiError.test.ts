...
describe('useApiError', () => {
  ...
  it('returns serviceLabel for Hermes Agent network error', () => {
    const err = new ApiError(0, 'timeout', '/api/chat/history')
    const { result } = renderHook(() => useApiError(err))
    expect(result.current).toBe('Hermes Agent unreachable')
  })
  ...
  it('returns "API {status}" with service context for HTTP errors', () => {
    ...
    const { result } = renderHook(() => useApiError(err))
    expect(result.current).toBe('API 502 — Hermes Agent unreachable')
  })
