export type CodexLbUsageWindowId = 'fiveHour' | 'weekly'

export interface CodexLbUsageWindow {
  id: CodexLbUsageWindowId
  label: string
  used?: number
  limit?: number
  remaining?: number
...
export interface CodexLbUsageAccount {
  id: string
  label: string
  used?: number
  limit?: number
  ...
  status?: string
  resetAt?: string
  windows: CodexLbUsageWindow[]
}

export interface CodexLbUsageSummary {
  raw: unknown
  totalTokens?: number
  promptTokens?: number
...
function withDerivedUsage(values: {
  ...
  remaining?: number
  percent?: number
}): Pick<CodexLbUsageWindow, 'used' | 'limit' | 'remaining' | 'percent'> {
...
function readUsageNumbers(record: RecordValue): Pick<CodexLbUsageWindow, 'used' | 'limit' | 'remaining' | 'percent'> {
...
function findWindowSource(record: RecordValue, id: CodexLbUsageWindowId): RecordValue | null {
...
function windowFromRecord(record: RecordValue, id: CodexLbUsageWindowId): CodexLbUsageWindow | null {
...
function windowsFromRecord(record: RecordValue): CodexLbUsageWindow[] {
  return [
    windowFromRecord(record, 'fiveHour'),
    windowFromRecord(record, 'weekly'),
  ].filter((window): window is CodexLbUsageWindow => Boolean(window))
}
...
function accountFromEntry(entryKey: string, value: unknown): CodexLbUsageAccount | null {
...
function accountsFromUsage(record: RecordValue): CodexLbUsageAccount[] {
  ...
  return accountEntries(source)
    ...
    .filter((account): account is CodexLbUsageAccount => Boolean(account))
}

export function normalizeCodexLbUsage(raw: unknown): CodexLbUsageSummary | null {
  ...
  const summary: CodexLbUsageSummary = {
    raw,
    totalTokens,
...
export function formatCodexUsageNumber(value: number | undefined, suffix = ''): string {
...
export function formatCodexUsageCost(value: number | undefined): string {
  return value === undefined ? '--' : `$${value.toFixed(2)}`
}

export function formatCodexUsagePercent(value: number | undefined): string {
  return value === undefined ? '--' : `${Math.round(value)}%`
}

export function formatCodexUsageReset(value: string | undefined): string {
  if (!value) return '--'
...
export function codexUsageCompactLabel(summary: CodexLbUsageSummary | null): string {
  if (!summary) return 'Codex LB'
  const fiveHour = summary.windows.find((window) => window.id === 'fiveHour')
  if (fiveHour?.percent !== undefined) return `5h ${formatCodexUsagePercent(fiveHour.percent)}`
  if (summary.remaining !== undefined) return `${formatCodexUsageNumber(summary.remaining)} left`
  if (summary.used !== undefined) return `${formatCodexUsageNumber(summary.used)} used`
  if (summary.totalTokens !== undefined) return `${formatCodexUsageNumber(summary.totalTokens)} tok`
  if (summary.totalCost !== undefined) return formatCodexUsageCost(summary.totalCost)
  return 'Codex LB'
}
