...
const TRACKED_STORAGE_KEY = 'job-hunter-tracked-leads'
const REVIEW_QUEUE_STORAGE_KEY = 'job-hunter-review-queue'
const FEEDBACK_STORAGE_KEY = 'job-hunter-feedback'
const SAVED_SEARCHES_STORAGE_KEY = 'job-hunter-saved-searches'
const LIFE_MODE_STORAGE_KEY = 'job-hunter-life-mode'
...
const DEFAULT_SOURCE_KEYS: SearchSourceKey[] = ['remotive', 'remoteok', 'arbeitnow']
const BROWSER_REVIEW_MAX_AGE_DAYS = 14
const BROWSER_REVIEW_MIN_SALARY_BUFFER = 0
const BROWSER_REVIEW_KEYWORDS = /ai|automation|data|engineer|developer|intern|entry/
const LOW_SIGNAL_SCORE_FLOOR = 25
...
const FEEDBACK_SCORE: Record<JobFeedback, number> = {
  good: 12,
  bad: -30,
  applied: 10,
  ignored: -16,
...
const LIFE_MODE_CONFIG: Record<LifeMode, {
  label: string
  blurb: string
  defaultQuery: string
  searchLimit: number
...
function loadTrackedLeads(): TrackedLead[] {
...
function loadFeedback(): Record<string, JobFeedback> {
  if (typeof window === 'undefined') return {}
  try {
...
function loadLifeMode(): LifeMode {
...
function buildJobAnalysis(job: LiveJob, minimumHourlyRate: number, feedback?: JobFeedback): JobAnalysis {
...
function badgeStyle(stage: StageId, priority?: JobPriority): CSSProperties {
...
function leadFromJob(job: LiveJob): TrackedLead {
  return {
    id: createId(),
    company: job.company,
    role: job.title,
...
function reviewQueueFromJob(job: LiveJob, mode: ReviewQueueMode): ReviewQueueItem {
  const recommendation = recommendApplication(job)
...
function shouldAutoQueueBrowserReview(job: LiveJob, minimumHourlyRate: number): boolean {
  const recommendation = recommendApplication(job)
...
function buildSearchQuery(query: string, mode: WorkMode): string {
...
function serializeSources(sources: SearchSourceKey[]): string {
  return (sources.length > 0 ? sources : DEFAULT_SOURCE_KEYS).join(',')
}

function toggleSource(sources: SearchSourceKey[], source: SearchSourceKey): SearchSourceKey[] {
...
function estimateHourlyRate(salary?: string | null): number | null {
...
function buildLinkedInSearchUrl(job: LiveJob): string {
...
function buildGoogleSearchUrlFromText(query: string, location: string): string {
  const text = [
    query,
    location,
    'site:boards.greenhouse.io OR site:jobs.lever.co OR site:myworkdayjobs.com OR site:jobs.ashbyhq.com OR site:jobs.workable.com',
...
function recommendApplication(job: LiveJob): ApplyRecommendation {
  const title = `${job.title} ${job.category}`.toLowerCase()
  const salary = estimateHourlyRate(job.salary)
...
function applyModeStyle(mode: ApplyMode): CSSProperties {
  switch (mode) {
    case 'direct':
...
function applyModeRank(mode: ApplyMode): number {
  switch (mode) {
    case 'direct':
      return 0
    case 'manual':
