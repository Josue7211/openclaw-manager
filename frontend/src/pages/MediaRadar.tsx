...
interface RequestDiscoveryProvider {
  id: string
  name: string
}

interface RequestDiscoveryResponse extends SearchResponse {
...
type MediaView = 'overview' | 'browse' | 'add' | 'downloads' | 'requests' | 'missing' | 'indexers' | 'setup' | 'library'
type WorkflowTarget = Exclude<MediaView, 'overview'>
type ServiceSetupFilter = 'attention' | 'online' | 'detected' | 'configured' | 'all'
type RequestStatusFilter = 'pending' | 'all' | 'approved' | 'available' | 'partial' | 'declined'
type SubtitleKindFilter = 'all' | 'movies' | 'episodes'
type IndexerStateFilter = 'all' | 'enabled' | 'disabled'
type LibraryMonitorFilter = 'all' | 'monitored' | 'unmonitored'
type DiscoverKindFilter = 'tv' | 'movie'
type DiscoverCategoryFilter = 'popular' | 'trending' | 'upcoming'
...
function libraryKind(item: LibraryItem): string {
...
function libraryNetwork(item: LibraryItem): string {
  return item.network ?? item.studio ?? item.genres?.[0] ?? 'No network'
}

function librarySearchText(item: LibraryItem): string {
  return [
    libraryTitle(item),
    libraryKind(item),
    libraryNetwork(item),
...
function itemDetailRef(item: { service?: string; kind?: string; id?: number | string; detail_id?: number | string; detail_ref?: DetailRef } | null | undefined): DetailRef | null {
  if (!item) return null
...
function streamPlayer(item: StreamItem): string {
  return item.player ?? item.friendly_name ?? item.product ?? item.state ?? 'playing'
}

function streamDecision(item: StreamItem): string {
...
function requestIsPending(item: RequestItem): boolean {
...
function downloadMeta(item: DownloadItem): string {
  const speeds = [
...
function subtitleLabel(item: SubtitleItem): string {
...
function subtitleKind(item: SubtitleItem): 'movies' | 'episodes' {
  return item.radarrId ? 'movies' : 'episodes'
}

function subtitleLanguage(item: SubtitleItem): string {
...
function releaseMeta(item: Record<string, unknown>): string {
  const parts = [
    item.indexer,
    item.protocol,
...
function requestSeasonNumbers(item: Record<string, unknown>): number[] {
...
function requestResultKey(item: Record<string, unknown>): string {
  return String(item.id ?? `${resultTitle(item)}-${item.mediaType ?? item.media_type ?? 'media'}`)
}

function parseSeasonSelection(value: string): number[] {
  return Array.from(
    new Set(
      value
        .split(',')
...
function serviceStateLabel(service: MediaService): string {
...
function serviceStateColor(service: MediaService, accent: string): string {
...
function serviceNeedsAttention(service: MediaService): boolean {
...
function serviceAttentionRank(service: MediaService): number {
...
function serviceIssueText(service: MediaService): string {
...
function serviceSettingsUrl(service: MediaService): string {
...
function serviceGroupLabel(service: MediaService): string {
...
function serviceGroupRank(label: string): number {
