...
const GeneratedModuleCard = memo(function GeneratedModuleCard({ module }: { module: GeneratedModule }) {
  ...
  const [versions, setVersions] = useState<GeneratedModuleVersion[]>([])
  const [loadingVersions, setLoadingVersions] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState(false)

  const toggleMut = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) => toggleGeneratedModule(id, enabled),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: queryKeys.generatedModules }),
  })

  const deleteMut = useMutation({
    mutationFn: (id: string) => deleteGeneratedModule(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: queryKeys.generatedModules }),
  })

  const rollbackMut = useMutation({
    mutationFn: ({ id, version }: { id: string; version: number }) => rollbackGeneratedModule(id, version),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.generatedModules })
      getGeneratedModuleVersions(module.id).then(setVersions)
    },
  })
  ...
  const handleToggleHistory = async () => {
    if (!showHistory && versions.length === 0) {
      ...
      try {
        const v = await getGeneratedModuleVersions(module.id)
        setVersions(v)
      } catch {
...
export function GeneratedModulesSection() {
  const { data, isLoading } = useQuery({
    queryKey: queryKeys.generatedModules,
    queryFn: () => api.get<{ modules: GeneratedModule[] }>('/api/generated-modules'),
  })
  ...
  return (
    <div style={{ marginTop: '24px' }}>
      ...
      {isLoading ? (
        ...
        <div>
          {modules.map(mod => (
            <GeneratedModuleCard key={mod.id} module={mod} />
          ))}
        </div>
      )}
    </div>
