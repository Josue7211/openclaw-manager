...
function usageDetailLine(summary: CodexLbUsageSummary | null): string {
  if (!summary) return 'No usage data yet'
  if (summary.remaining !== undefined && summary.limit !== undefined) {
    return `${formatCodexUsageNumber(summary.remaining)} remaining of ${formatCodexUsageNumber(summary.limit)}`
  }
  if (summary.used !== undefined && summary.limit !== undefined) {
    return `${formatCodexUsageNumber(summary.used)} used of ${formatCodexUsageNumber(summary.limit)}`
  }
  if (summary.totalTokens !== undefined) return `${formatCodexUsageNumber(summary.totalTokens)} tokens`
  if (summary.totalCost !== undefined) return `${formatCodexUsageCost(summary.totalCost)} total`
...
function WindowMeter({ window }: { window: CodexLbUsageWindow }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 3, minWidth: 76 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 6, fontSize: 10, color: 'var(--text-muted)' }}>
        ...
        <span>{formatCodexUsagePercent(window.percent)}</span>
      </div>
      <div style={{
        height: 4,
        borderRadius: 999,
...
function AccountRow({ account }: { account: CodexLbUsageAccount }) {
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ color: 'var(--text-muted)', fontSize: 11, marginTop: 2 }}>
          {account.remaining !== undefined
            ? `${formatCodexUsageNumber(account.remaining)} left`
            : account.used !== undefined
              ? `${formatCodexUsageNumber(account.used)} used`
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        {!fiveHour && !weekly && account.percent !== undefined && (
          <span style={{ color: 'var(--text-secondary)', fontSize: 12, fontFamily: 'monospace' }}>
            {formatCodexUsagePercent(account.percent)}
          </span>
        )}
      </div>
    </div>
...
function UsagePopover({
  ...
  summary: CodexLbUsageSummary | null
  loading: boolean
  fetching: boolean
  error: Error | null
  fromCache: boolean
  ...
  return (
    <div
      role="dialog"
      aria-label="Codex LB usage details"
      style={{
        position: 'absolute',
        right: 0,
        bottom: 34,
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        <div>
          <div style={{ color: 'var(--text-primary)', fontSize: 13, fontWeight: 700 }}>Codex LB usage</div>
          <div style={{ color: 'var(--text-muted)', fontSize: 11, marginTop: 2 }}>
        ...
        <button
          type="button"
          onClick={onRefresh}
          aria-label="Refresh Codex LB usage"
          title="Refresh Codex LB usage"
          style={{
            width: 28,
            height: 28,
            borderRadius: 8,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 8 }}>
        <UsageStat label="Used" value={formatCodexUsageNumber(summary?.used ?? summary?.totalTokens)} />
        <UsageStat label="Remaining" value={formatCodexUsageNumber(summary?.remaining)} />
        <UsageStat label="Cost" value={formatCodexUsageCost(summary?.totalCost)} />
      </div>
      ...
      {(summary?.period || summary?.resetAt) && (
        <div style={{ color: 'var(--text-muted)', fontSize: 11 }}>
          ...
          {summary.period && summary.resetAt ? ' · ' : ''}
          {summary.resetAt ? `Resets ${formatCodexUsageReset(summary.resetAt)}` : ''}
        </div>
      )}

      {error && (
        <div role="status" style={{ color: 'var(--amber)', fontSize: 11 }}>
          {fromCache ? 'Showing cached usage while refresh fails.' : 'Codex LB usage is unavailable.'}
        </div>
      )}
      ...
      {summary?.accounts.length ? (
        ...
        <div style={{ color: 'var(--text-muted)', fontSize: 11 }}>
          Account breakdown not returned by Codex LB.
        </div>
      )}

      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
        <PopoverLink onClick={() => onNavigate('/settings?section=usage')}>Usage</PopoverLink>
        <PopoverLink onClick={() => onNavigate('/settings?section=codex-lb')}>Codex LB</PopoverLink>
      </div>
    </div>
  )
}
...
export default function CodexLbUsagePill() {
  const [open, setOpen] = useState(false)
  const wrapperRef = useRef<HTMLDivElement>(null)
  const navigate = useNavigate()
  const { usage, loading, fetching, error, fromCache, lastUpdatedAt, refetch } = useCodexLbUsage()
  const windows = useMemo(() => usage?.windows ?? [], [usage])
  const showWindowMeters = windows.length > 0
  const label = showWindowMeters ? 'Codex LB' : codexUsageCompactLabel(usage)
  ...
  return (
    <div ref={wrapperRef} style={{ position: 'relative', marginLeft: 'auto', flexShrink: 0 }}>
      ...
      >
        ...
        {showWindowMeters && (
          <span className="codex-lb-window-meters" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {windows.map((window) => <WindowMeter key={window.id} window={window} />)}
          </span>
        )}
        <span style={{
