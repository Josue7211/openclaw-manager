/**
 * Sandboxed iframe preview for generated modules.
 *
 * Security layers:
 ...
 * /api/generated-modules/bridge → response sent back via postMessage.
 */
...
export function ModulePreview({
  ...
  useEffect(() => {
    function handleMessage(event: MessageEvent) {
      ...
      api.post('/api/generated-modules/bridge', { source: reqSource, command })
        .then((result) => {
  ...
  if (structuredPreview) {
    return (
      <div style={containerStyle}>
        <PreviewHeader />
        <div style={structuredPreviewStyle}>
          {structuredPreview}
        </div>
      </div>
    )
...
function renderProposalNode(node: PrimitiveNode, key: string): React.ReactNode {
  const Primitive = PRIMITIVE_COMPONENTS[node.primitive as keyof typeof PRIMITIVE_COMPONENTS]
  if (!Primitive) {
    return (
      <div key={key} style={unknownPrimitiveStyle}>
        Unknown primitive: {node.primitive}
      </div>
    )
  }
...
const structuredPreviewStyle: React.CSSProperties = {
  flex: 1,
  minHeight: 0,
  overflow: 'auto',
  padding: '12px',
...
const unknownPrimitiveStyle: React.CSSProperties = {
  padding: '12px',
  borderRadius: '12px',
