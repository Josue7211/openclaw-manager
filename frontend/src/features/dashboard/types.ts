...
export function effortColor(value: string | null): string {
  return value === 'low' ? 'var(--secondary)' : value === 'medium' ? 'var(--amber)' : value === 'high' ? 'var(--red-bright)' : 'var(--text-muted)'
}

export function pillStyle(value: string | null): React.CSSProperties {
  return {
    ...
    fontSize: '10px',
    fontWeight: 600,
    background: `${effortColor(value)}22`,
    color: effortColor(value),
    border: `1px solid ${effortColor(value)}44`,
